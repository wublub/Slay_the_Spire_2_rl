...
public class NTreasureRoomRelicCollection : Control, IScreenContext
{
	...
	protected override void RestoreGodotObjectData(GodotSerializationInfo info)
	{
		...
		if (info.TryGetProperty(PropertyName._hands, out var value4))
		{
			_hands = value4.As<NHandImageCollection>();
		}
		if (info.TryGetProperty(PropertyName._openedTicks, out var value5))
		{
			_openedTicks = value5.As<ulong>();
		}
		if (info.TryGetProperty(PropertyName._emptyLabel, out var value6))
		{
			_emptyLabel = value6.As<Label>();
		}
